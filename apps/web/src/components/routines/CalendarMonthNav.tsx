...
export function CalendarMonthNav({ month }: { month: MonthInfo }) {
  const router = useRouter();
  const [, startTransition] = useTransition();

  function goto(anchor: Date | null) {
    startTransition(() => {
      if (!anchor) router.push('/routines/calendar');
      else router.push(`/routines/calendar?month=${toISOMonth(anchor)}`);
    });
  }

  const today = new Date();
  const isThisMonth =
    month.year === today.getFullYear() && month.month === today.getMonth() + 1;

  return (
    <div className="flex items-center gap-2">
      <button
        type="button"
        onClick={() => goto(addMonths(month.firstOfMonth, -1))}
