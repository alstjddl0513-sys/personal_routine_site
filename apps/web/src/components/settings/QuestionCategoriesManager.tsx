...
export function QuestionCategoriesManager({
  initial,
}: {
  initial: QuestionCategory[];
}) {
  const router = useRouter();
  const [rows, setRows] = useState<QuestionCategory[]>(initial);
  const [edits, setEdits] = useState<Record<string, EditState>>({});
  const [error, setError] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();
  const [deleteTarget, setDeleteTarget] = useState<QuestionCategory | null>(null);

  function beginEdit(row: QuestionCategory) {
  ...
  function saveEdit(row: QuestionCategory) {
    ...
    startTransition(async () => {
      try {
        const updated = await patchQuestionCategory(row.id, { label });
  ...
  function confirmDelete() {
    ...
    startTransition(async () => {
      try {
        await deleteQuestionCategory(target.id);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <p className="text-xs text-zinc-500 dark:text-zinc-400">
        질문과 연결되는 <strong>key</strong>는 만든 뒤에는 바꿀 수 없어요. 라벨은 언제든 다시 지어도 괜찮아요.
      ...
      <ConfirmDialog
        ...
        description={
          <p>
            ...
            {' '}카테고리를 삭제할까요? 이미 이 카테고리를 쓰던 질문은 그대로 남고, 필터 목록에서만 사라져요.
          </p>
        }
...
function AddRow({
  ...
  onAdd: (row: QuestionCategory) => void;
  ...
  function submit(e: FormEvent) {
    ...
    const l = label.trim();
    if (!/^[a-z0-9_]+$/.test(k)) {
      onError('key는 소문자/숫자/언더스코어만 사용 (예: my_topic)');
      return;
    }
    if (!l) {
      onError('라벨을 입력하세요.');
    ...
    startTransition(async () => {
      try {
        const created = await createQuestionCategory({ key: k, label: l });
        onAdd(created);
        reset();
        router.refresh();
      } catch (err) {
